...
export function Method({ item, language, icons }: SceneProps & { icons: LucideIcon[] }) {
  return <ol className="scene-method">{item.actions.map((action, index) => <li key={index}><IconBadge icon={icons[index]} quiet /><p>{action[language]}</p></li>)}</ol>
}
